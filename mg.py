...
def updateChannelUrlsM3U(channels, template_channels):
    ...
    with open("mg.m3u", "w", encoding="utf-8") as f_m3u:
        f_m3u.write("#EXTM3U\n")

        with open("mg.txt", "w", encoding="utf-8") as f_txt:
            for category, channel_list in template_channels.items():
                    ...
                    for channel_name in channel_list:
                        if channel_name in channels[category]:
                            for url in channels[category][channel_name]:
                                if url and url not in written_urls and not any(blacklist in url for blacklist in config.url_blacklist):  # Check if 网站 is not already written and not in blacklist
                                    ...
                                    written_urls.add(url)  # Add 网站 to written URLs set

            f_txt.write("\n")

if __name__ == "__main__":
    template_file = "demo3.txt"
